...
- **Supabase Auth 사용**
- 로그인 방식은 우선 `email magic link`
- 이후 필요 시 `Sign in with Apple` 추가

이유:

- 직접 인증을 구현하면 비밀번호 재설정, 이메일 인증, refresh token 보안, 계정 복구 비용이 커진다
- 현재 프로젝트 규모에서는 외부 auth 서비스가 더 빠르고 안전하다
...
1. 설정 화면에서 계정 삭제 진입
2. 재확인
3. 서버에서 soft delete 또는 hard delete 정책 수행
4. 로컬 세션 제거
5. 로그인 화면 복귀
